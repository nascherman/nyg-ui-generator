...
var getModuleLibrary = require('./getModuleLibrary');
    ...
    .on('precopy', function () {
      ...
      if (!flags.location) {
        gen.prompt({
          ...
          getModuleLibrary(type, function(modules) {
              var names = getModuleNames(modules);
              var choices = createChoices(names);
              console.log('choices');
              console.log(choices);
              gen.prompt({
                type: 'list',
                message: 'Select a ui template',
                name: 'template',
                ...
                var type = gen.config.get('type');
                if(gen.config.get('template') !== 'default') {
                  gen.config.set('type', type + '-module');
                }
                done();
              });
            });
...
function getModuleNames(modules) {
  ...
  modules.forEach(function(module) {
    names.push(module.key[1]);
  });
  return names;
}

function createChoices(names) {
  var choices = [];
